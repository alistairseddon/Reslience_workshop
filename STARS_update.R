...
stars<- function(y=c(rnorm(50), rnorm(50,2,1)), time = seq(1:100), L=20, p=0.05, h=1,  AR1red="none", prewhitening = F) {
	...
	if(length(time) != length(y)) {
		stop("length of time series ('y' arg) different to length of time units ('time' argument)")
	} else {
		names(y) <- time
	}
...
plot.stars<-function(starsOb){
 	
 	starsPlot = starsOb$starsResult
 	alpha = starsOb$alpha
 	
 	age <- as.numeric(rownames(starsPlot))
 	 	
 	par(mfrow=c(2,1))
 	plot(age, starsPlot[,1], type="o", cex=0.3, lty=3, col="lightblue", lwd=0.8, xlab=" ", ylab=colnames(starsPlot)[1])
	points(age, starsPlot[,2], type= "o", cex=0.3, lty=3, col="darkblue", pch=1)	
	points(age, starsPlot[,4], type="l", col="red", lty=1, lwd=1.5)
	if(alpha>0) {
	...
	plot(age, abs(starsPlot[,3]), type="h", lty=1, lwd=3, xlab=" Age (cal yr BP)", ylim=c(0, max(abs(starsPlot[,3]), na.rm=T)*1.1), col="red", ylab="RSI")
